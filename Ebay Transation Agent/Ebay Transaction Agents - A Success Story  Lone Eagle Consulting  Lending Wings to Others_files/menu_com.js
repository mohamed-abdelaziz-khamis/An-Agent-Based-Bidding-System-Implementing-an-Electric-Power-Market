/*(c) Ger Versluis 2000 version 9.20  24 April 2003
You may use this script on non commercial sites. 
For info write to menus@burmees.nl*/
if(typeof(StartTop)!="undefined"&&typeof(StartLeft)!="undefined"){
var AgntUsr=navigator.userAgent.toLowerCase(),AppVer=navigator.appVersion.toLowerCase();
var DomYes=document.getElementById?1:0,NavYes=AgntUsr.indexOf("mozilla")!=-1&&AgntUsr.indexOf("compatible")==-1?1:0,ExpYes=AgntUsr.indexOf("msie")!=-1?1:0;
var Opr7=AgntUsr.indexOf("opera 7")!=-1||AgntUsr.indexOf("opera/7")!=-1?1:0,Opr=(AgntUsr.indexOf("opera")!=-1&&!Opr7)?1:0;
...
var Fltr=(AppVer.indexOf("msie 6")!= -1||AppVer.indexOf("msie 7")!= -1)&&!Opr7?1:0;
var MacExp5=(MacCom&&Exp5)?1:0;
var PosStrt=((NavYes||ExpYes||Opr7)&&!Opr)?1:0;
var RmbrNow=null,FLoc,ScLoc,DcLoc,SWinW,SWinH,FWinW,FWinH,SLdAgnWin,FColW,SColW,DColW,RLvl=0,FrstCreat=1,Ldd=0,Crtd=0,IniFlg,AcrssFrms=1,FrstCntnr=null,CurOvr=null,CloseTmr=null,CntrTxt,TxtClose,ImgStr,ShwFlg=0,M_StrtTp=StartTop,M_StrtLft=StartLeft,StaticPos=0;
if(Opr7)LeftPaddng=TopPaddng=0;
var LftXtra=DomNav?LeftPaddng:0,TpXtra=DomNav?TopPaddng:0;
var FStr="",M_Hide=Nav4?"hide":"hidden",M_Show=Nav4?"show":"visible",Par=MenuUsesFrames?parent:window,Doc=Par.document,Bod=Doc.body,LastHref="";
var Trigger=NavYes&&!Opr7?Par:Bod;
var Ztop=100,InitLdd=0,P_X=DomYes?"px":"";
var OpnTmr=null;

	if((StartTop>=0)&&PosStrt){
...
function UnLddTotal(){MakeNull(FrstCntnr);FrstCntnr=RmbrNow=FLoc=ScLoc=DcLoc=SLdAgnWin=CurOvr=CloseTmr=Doc=Bod=Trigger=OpnTmr=null}
...
function ChckLdd(){
	if(!ExpYes&&!Opr7){if(ScLoc.document.body){if(LdTmr)clearInterval(LdTmr);Go()}}
...
function Go(){
	if(!Ldd&&PosStrt){
		...
		if(MenuUsesFrames){
			SLdAgnWin=!NavYes||Opr7?ScLoc.document.body:ScLoc;
			SLdAgnWin.onunload=Nav4?NavUnLdd:UnLdd}
		if(ExpYes&&!Opr7)Trigger.onunload=UnLddTotal;
...
function GoTo(){
	...
	if(HP.Arr[1]){status="";LowItem(HP);IniFlg=1;Initiate();
		if(HP.Arr[1].indexOf("javascript:")!=-1)eval(HP.Arr[1]);
		else if(BaseHref+HP.Arr[1]!=LastHref){LastHref=BaseHref+HP.Arr[1];if(HP.Arr[17]==1){window.open(BaseHref+HP.Arr[1],HP.Arr[0]);}else{DcLoc.location.href=BaseHref+HP.Arr[1]}}}}
...
function MbrSetUp(MbC,PrMmbr,WMu,Wd,Ht,Nofs){
	...
	if(Tfld.indexOf("rollover")!=-1){this.ro=1;this.ri1=Tfld.substring(Tfld.indexOf("?")+1,Tfld.lastIndexOf("?"));
		...
		Tfld="<img src=\""+this.ri1+"\" name=\""+this.rid+"\" width=\""+Wd+"\" height=\""+Ht+"\" alt=\"\">"}
	this.value=Tfld;
	...
	if(this.Arr[3]){a=RLvl==1&&FirstLineHorizontal?BottomUp?9:3:RightToLeft?6:0;
		if(Arrws[a]!=""){S=Arrws[a];W=Arrws[a+1];H=Arrws[a+2];T=RLvl==1&&FirstLineHorizontal?BottomUp?2:Ht-H-2:(Ht-H)/2;L=RightToLeft?2:Wd-W-2;
			...
			else{Tfld+="<div id=\""+WMu+"_im\" style=\"position:absolute; top:"+T+"; left:"+L+"; width:"+W+"; height:"+H+";visibility:inherit\"><img src=\""+S+"\" alt=\"\"></div>";
				this.innerHTML=Tfld;t=Lctn.document.all[WMu+"_im"]}
			this.ai=t}}
	if(ExpYes||Opr7){this.onselectstart=CnclSlct;this.onmouseover=RLvl==1&&UnfoldsOnClick?OpenMenuClick:OpenMenu;
...
function NavMbrSetUp(MbC,PrMmbr,WMu,Wd,Ht,Nofs){
	...
	if(this.value.indexOf("rollover")!=-1){
		...
		this.value="<img src=\""+this.ri1+"\" name=\""+this.rid+"\" alt=\"\">"}
	...
	if(this.Arr[3]){a=RLvl==1&&FirstLineHorizontal?BottomUp?9:3:RightToLeft?6:0;
		if(Arrws[a]!=""){this.CLyr.ILyr=new Layer(Arrws[a+1],this.CLyr);this.CLyr.ILyr.visibility="inherit";
			...
			ImgStr="<img src=\""+Arrws[a]+"\" width=\""+Arrws[a+1]+"\" height=\""+Arrws[a+2]+"\" alt=\"\">";
